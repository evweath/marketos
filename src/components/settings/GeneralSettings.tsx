...
export function GeneralSettings() {
  // Business info
  const [businessName, setBusinessName] = useState(GENERAL_SETTINGS.businessName);
  const [timezone, setTimezone] = useState(GENERAL_SETTINGS.timezone);
  const [currency, setCurrency] = useState(GENERAL_SETTINGS.currency);
  const [fiscalMonth, setFiscalMonth] = useState(GENERAL_SETTINGS.fiscalYearStartMonth);
  const [dateFormat, setDateFormat] = useState(GENERAL_SETTINGS.dateFormat);
  const [language, setLanguage] = useState(GENERAL_SETTINGS.language);
  const [bizSaved, setBizSaved] = useState(false);

  // Branding
  const [brandColor, setBrandColor] = useState('#00d9ff');
  const [footerText, setFooterText] = useState('Generated by MarketOS · Confidential');
  ...
  const [recipientEmails, setRecipientEmails] = useState('rachel@marketosbakery.com, derek@marketosbakery.com');
  ...
  // Notifications
  const [notifications, setNotifications] = useState<NotificationRule[]>(NOTIFICATION_PREFERENCES);
      ...
      <div className="glass-card p-5">
        ...
        <div className="flex flex-col gap-4 mb-5">
          ...
          <div>
            ...
            <div className="flex items-center justify-center rounded-xl py-8 cursor-pointer transition-all hover:bg-white/[0.02]"
              style={{ border: '1px dashed var(--border-dim)' }}>
              <div className="flex flex-col items-center gap-2">
                <Upload size={18} style={{ color: 'var(--text-muted)' }} />
                <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Click to upload logo — PNG, SVG, up to 2MB</span>
              </div>
            </div>
          </div>
