...
export function KeywordRankTable() {
  ...
  function SortIcon({ k }: { k: SortKey }) {
    ...
    return sortAsc
      ? <ArrowUp size={10} style={{ color: '#00d9ff' }} />
      : <ArrowDown size={10} style={{ color: '#00d9ff' }} />;
  }
    ...
    <div className='glass-card p-4'>
            ...
            {filtered.map((kw, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td className='py-3 pr-3'>
                    <span
                      className='text-[10px] px-2 py-0.5 rounded font-mono font-semibold whitespace-nowrap'
                  ...
                  <td className='py-3 pr-3 text-center'>
                    <span
                      className='inline-flex items-center justify-center w-9 h-9 rounded-full font-mono text-sm font-bold'
                  ...
                  <td className='py-3 pr-3'>
                    <div className='flex items-center gap-2'>
                      ...
                      <span className='font-mono text-[10px] w-5 text-right tabular-nums' style={{ color: dColor }}>
                        {kw.difficulty}
                      </span>
                    </div>
                  </td>
      ...
      <div className='flex items-center gap-5 mt-4 pt-3' style={{ borderTop: '1px solid var(--border-subtle)' }}>
        ...
        {[
          ...
          { label: '#4–10', color: '#00d9ff' },
          { label: '#11–30', color: '#ffb347' },
          { label: '#30+',  color: '#ff4444' },
        ].map(r => (
          <div key={r.label} className='flex items-center gap-1.5'>
            <div className='w-2.5 h-2.5 rounded-full' style={{ background: r.color }} />
            <span className='text-[10px] font-mono' style={{ color: 'var(--text-muted)' }}>{r.label}</span>
          </div>
        ))}
      </div>
    </div>
